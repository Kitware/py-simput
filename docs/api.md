...
Base class for objects that can be used as value for properties.
They can be passed around by copy instead of reference. A proxy still provide the same reference on which you can interact with method calls but when the value needs to be shared to another location or service, the object get serialized and recreated on the other side in a transparent manner.
...
__Load language/layout definitions__
...
__Data exchange for UI handling__
...
User just need to call `apply_all()` after any batch update to flush the domain and set any missing property initialization.
